...
/**
 * Performance test for the SetupCompany entity.
 */
class SetupCompanyGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SetupCompany entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all setupCompanys")
            .get("/api/setupCompanys")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created setupCompany")
                .get("${new_setupCompany_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created setupCompany")
            .delete("${new_setupCompany_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
